...
{#snippet sidebarContent()}
  <div class="flex h-full flex-col">
    ...
    <!-- Messages -->
    <section bind:this={elemChat} class="max-h-[600px] space-y-4 overflow-y-auto p-4">
        ...
        {#each messages.slice(1) as message}
          {#if message.role === "user"}
            <div class="grid grid-cols-[1fr_auto] gap-2">
              <div class="card space-y-2 justify-self-end rounded-xl p-4 preset-tonal">
          ...
          {:else}
            <div class="grid grid-cols-[auto_1fr] gap-2">
              ...
              <div class="card p-4">
                <p>{@html marked(message.content)}</p>
                <button
                  on:click={() => updateMessageHelpful(message.responseId, false)}
  ...
  <!-- Prompt -->
  <section class="fixed bottom-0 left-0 z-10 w-[485px] p-4 border-surface-200-800">
    <!-- svelte-ignore element_invalid_self_closing_tag -->
    <textarea
      class="textarea preset-tonal"
      bind:value={inputMessage}
      on:keydown={handleKeyDown}
      placeholder="Message Tutors AI"
      rows={3}
    />
  </section>
{/snippet}
